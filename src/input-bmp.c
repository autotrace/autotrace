...
static void setMasksDefault(unsigned short biBitCnt, Bitmap_Channel *masks)
{
  switch (biBitCnt) {
  case 32:
...
static unsigned char *ReadImage(FILE *, int, int, unsigned char[256][3], int, int, int, int,
                                gboolean, const Bitmap_Channel *, at_exception_type *);
                           ...
                           gpointer msg_data, gpointer user_data)
{
  ...
  if (!ReadOK(fd, magick, 2) ||
      !(!strncmp(magick, "BA", 2) || !strncmp(magick, "BM", 2) || !strncmp(magick, "IC", 2) ||
        !strncmp(magick, "PT", 2) || !strncmp(magick, "CI", 2) || !strncmp(magick, "CP", 2))) {
  ...
  if (Bitmap_Head.biWidth < 0 || Bitmap_Head.biHeight == -2147483648) {
    LOG("%s is not a valid BMP file", filename);
    at_exception_fatal(&exp, "bmp: invalid input file");
    goto cleanup;
  }
...
static gboolean ReadColorMap(FILE *fd, unsigned char buffer[256][3], int number, int size,
                             gboolean *Grey, at_exception_type *exp)
{
  int i;
  unsigned char rgb[4];
...
static gboolean ReadChannelMasks(unsigned long *tmp, Bitmap_Channel *masks, unsigned int channels)
{
  unsigned int i;
...
#ifdef _DEBUG
    LOG4("Channel %d mask %08x in %d max_val %d\n", i, masks[i].mask, masks[i].shiftin,
         (int)masks[i].max_value);
#endif
  }
...
static unsigned char *ReadImage(FILE *fd, int width, int height, unsigned char cmap[256][3],
                                int ncols, int bpp, int compression, int rowbytes, gboolean Grey,
                                const Bitmap_Channel *masks, at_exception_type *exp)
{
  unsigned char v, n;
  int xpos = 0;
  int ypos = 0;
  ...
  if (!(compression == BI_RGB || (bpp == 8 && compression == BI_RLE8) ||
        (bpp == 4 && compression == BI_RLE4) || (bpp == 16 && compression == BI_BITFIELDS) ||
    ...
    while (ReadOK(fd, row_buf, rowbytes)) {
      ...
      for (xpos = 0; xpos < width; ++xpos) {
        ...
        unsigned char red = *(temp++) =
            ((px32 & masks[0].mask) >> masks[0].shiftin) * 255.0 / masks[0].max_value + 0.5;
        unsigned char green = *(temp++) =
            ((px32 & masks[1].mask) >> masks[1].shiftin) * 255.0 / masks[1].max_value + 0.5;
        unsigned char blue = *(temp++) =
            ((px32 & masks[2].mask) >> masks[2].shiftin) * 255.0 / masks[2].max_value + 0.5;
    ...
    while (ReadOK(fd, row_buf, rowbytes)) {
      ...
      for (xpos = 0; xpos < width; ++xpos) {
        ...
        *(temp++) = ((rgb & masks[2].mask) >> masks[2].shiftin) * 255.0 / masks[2].max_value + 0.5;
        /* currently alpha channels are not supported by AutoTrace, thus simply ignored */
        /*if (channels > 3)
         *(temp++) = ((rgb & masks[3].mask) >> masks[3].shiftin) * 255.0 / masks[3].max_value +
         * 0.5;*/
      }
        ...
        /* Count + Color - record */
        {
          ...
          for (j = 0; ((unsigned char)j < (unsigned char)row_buf[0]) && (xpos < width);) {
            ...
            for (i = 1; ((i <= (8 / bpp)) && (xpos < width) &&
              ...
              *temp = (unsigned char)((row_buf[1] & (((1 << bpp) - 1) << (8 - (i * bpp)))) >>
                                      (8 - (i * bpp)));
              if (Grey)
                *temp = cmap[*temp][0];
            }
