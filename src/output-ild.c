...
int find_best_match_color(unsigned char r, unsigned char g, unsigned char b){
  ...
  for(i = 0; i < 255; i++) {
    t = r - ilda_standard_color_palette[i][0]; d  = t*t;
    t = g - ilda_standard_color_palette[i][1]; d += t*t;
    t = b - ilda_standard_color_palette[i][2]; d += t*t;
    if(d < dmin) {
      dmin = d; ret = i;
      if(dmin == 0) return ret;
    }
  }
  return ret;
}
...
pLaserPoint newLaserPoint(void){
  ...
  if ((p = (pLaserPoint)malloc(sizeof(LaserPoint))) == NULL) return(NULL);
...
pLaserFrame newLaserFrame(void){
  ...
  if ((p = (pLaserFrame)malloc(sizeof(LaserFrame))) == NULL)  return(NULL);
...
int writeILDAFrame(FILE *file, LaserFrame *f, int format){
  ...
  while(point){
    ...
    if((point->r == lastr) && (point->g == lastg) && (point->b == lastb)) {
      ...
      lastc = c; lastr = point->r; lastg = point->g; lastb = point->b;
    }
    ...
    if(points + 1 == cpoints) b += 0x80; // set last point bit
...
int writeILDAFrameHeader(FILE *file, LaserFrame *f, int format, unsigned int frames, unsigned int cframes){
  ...
  if(f) cpoints = frame_point_count(f);
...
void blankingPath(int x1, int y1, int x2, int y2){
  ...
  for(i = 0; i <= steps; i++){
    ...
    p->z = 0;
    p->r = 0; p->g = 0; p->b = 0; p->attrib = POINT_ATTRIB_BLANKED;
  }
}

/** No descriptions */
void blankingPathTo(int x, int y){
  if((!drawframe) || (!drawframe->point_last)) return;
...
void frameDrawFinish(){
  ...
  if(fromToZero) blankingPathTo(0, 0);
  ...
  if(insert_anchor_points) insertAnchorPoints();
}
...
void drawCubicBezier(double x1, double y1, double cx1, double cy1, double cx2, double cy2, double x2, double y2,
  ...
  for(this_list = 0; this_list < SPLINE_LIST_ARRAY_LENGTH(shape); this_list++){
    ...
    for (this_spline=0; this_spline<SPLINE_LIST_LENGTH(curr_list); this_spline++){
      ...
      switch((polynomial_degree)last_degree){
        case LINEARTYPE:
           //output Line
           drawLine((LastPoint.x - ox) * sx, (LastPoint.y - oy) * sy,(END_POINT(curr_spline).x - ox) * sx, (END_POINT(curr_spline).y - oy) * sy,
