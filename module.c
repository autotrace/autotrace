...
{     
  ...
  at_input_add_handler ("PBM", "Portable bitmap format",         input_pnm_reader);
  at_input_add_handler ("PNM", "Portable anymap format",         input_pnm_reader);
  at_input_add_handler ("PGM", "Portable graymap format",        input_pnm_reader);
  at_input_add_handler ("PPM", "Portable pixmap format",         input_pnm_reader);
...
#define AT_MAGICK_SET_INFO(X)			\
...
#endif	/* (MagickLibVersion < 0x0534) */
  {
	...
	if (info->AT_MAGICK_SUFFIX_FIELD_NAME && info->description)
	  at_input_add_handler(info->AT_MAGICK_SUFFIX_FIELD_NAME,
			       info->description,
			       input_magick_reader);
	info = info->next ;
      }
